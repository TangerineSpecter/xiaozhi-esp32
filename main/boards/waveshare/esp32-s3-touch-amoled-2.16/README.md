...
- 上方 `GPIO18` / 下方 `BOOT` 在 2×2 菜单中循环选择，当前提供“音量”“录音／停止录音”“记录／停止记录”和“文件”。文件页可播放最近三条录音或查看最近三条文字记录。
- 人物主界面长按上方 `GPIO18` 约 1 秒可直接开始录音；录音中再次长按会停止并保存。菜单中不触发该快捷键，文字记录中会提示先结束记录，屏幕休眠时第一次长按只负责唤醒。
- 主界面亮屏时点击角色头部会显示约 1 秒的摸头表情；菜单、录音、文字记录或眩晕反馈期间不会触发。
- 进入音量页后，上键增加、下键减少，每次 5%，范围 0–100%；中键确认后才应用并写入原有 `audio/output_volume` 设置。0% 静音在本板重启后保留。
- 音量页长按中键 2 秒取消未保存的修改。文件列表长按中键逐级返回；菜单右上角“关闭”可从任意层级直接回到人物主界面。
- 菜单使用遮罩和浅紫圆角图标卡片，180ms 淡入并轻微上浮。亮屏时可触摸菜单卡片、文件行、返回、关闭和音量条；休眠时触摸仍只负责唤醒。菜单外 BOOT 的对话、双击 AEC（启用时）及上下键组合重启保持原行为。
...
- 录音与文字记录互斥，启动其中一项前需要先结束另一项。录音中显示麦克风角色和 `REC mm:ss`；文字记录中显示记录状态。任一功能运行时暂停屏幕自动休眠和十分钟自动关机，停止后恢复原省电策略。
